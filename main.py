...
def run(credentials: dict, index: int):
    ...
    while True:
        if Client.config["commands"]["beg"] and data[username]:
            if (
                Client.config["cooldowns"]["patron"]
                and (
        ...
        if Client.config["commands"]["daily"] and data[username]:
            if (
                datetime.strptime(datetime.now().strftime("%x-%X"), "%x-%X")
                - datetime.strptime(Client.database["daily"], "%x-%X")
            ).total_seconds() > 23400:
                try:
                    daily(Client)
                except Exception:
        ...
        if Client.config["lottery"]["enabled"] and data[username]:
            if (
                datetime.strptime(datetime.now().strftime("%x-%X"), "%x-%X")
                - datetime.strptime(Client.database["lottery"], "%x-%X")
            ).total_seconds() > Client.config["lottery"]["cooldown"]:
                try:
                    lottery(Client)
                except Exception:
                    if Client.config["logging"]["warning"]:
        ...
        if Client.config["stream"]["enabled"] and data[username]:
            if (
                datetime.strptime(datetime.now().strftime("%x-%X"), "%x-%X")
                - datetime.strptime(Client.database["stream"], "%x-%X")
            ).total_seconds() > 600:
                try:
                    stream(Client)
                except Exception:
        ...
        if Client.config["commands"]["vote"] and data[username]:
            if (
                datetime.strptime(datetime.now().strftime("%x-%X"), "%x-%X")
                - datetime.strptime(Client.database["vote"], "%x-%X")
            ).total_seconds() > 43200:
                try:
                    vote(Client)
                except Exception:
